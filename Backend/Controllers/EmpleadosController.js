...
async function editarEmpleadoPorId(req, res) {
  // El ID viene de la URL (ej: /empleados/editar/15)
  const { idEmpleado } = req.params;

  // Los datos vienen en el body
  const {
    nombres, // ¡OJO! Tu hook lo llama 'nombre', asegúrate de que coincida
    apellidoPaterno,
    apellidoMaterno,
    diaNacimiento,
    mesNacimiento,
  ...
  // --- Validación de Fecha y Rol (igual que en guardar) ---
  if (!diaNacimiento || !mesNacimiento || !añoNacimiento) {
  ...
  try {
    ...
    await transaction.request()
      .input("idEmpleado", sql.Int, idEmpleado)
      .input("nombres", sql.NVarChar, nombres) // Asegúrate que el body envíe 'nombres'
      .input("apellidoPaterno", sql.NVarChar, apellidoPaterno)
      .input("apellidoMaterno", sql.NVarChar, apellidoMaterno)
      .input("fechaNacimiento", sql.Date, fechaNacimiento)
      .input("sexo", sql.NVarChar, sexo)
...
async function buscarEmpleadoPorUsuario(req, res) {
  ...
  try {
    ...
    // Búsqueda exacta (=) en lugar de LIKE, solo en nombre_usuario
    const result = await request
      .input("termino", sql.NVarChar, termino)
      .query(`
        SELECT 
          ...
          u.nombre_usuario, u.contraseña, r.nombre AS rol
        FROM Empleados e
        INNER JOIN Usuarios u ON e.id_empleado = u.id_empleado
        INNER JOIN Roles r ON u.id_rol = r.id_rol
    ...
    // Devolvemos un solo objeto 'empleado', no un array
    res.json({ success: true, empleado: result.recordset[0] });
  } catch (error) {
