  ...
  useEffect(() => {
    const checkDevice = async () => {
      ...
      const allowedAppleModels = [
        ...
        "iPhone 17", "iPhone 17 Pro", "iPhone 17 Pro Max", "iPad (10th generation)"
      ];

      const allowedSamsungModels = ["Samsung S25 Ultra", "Samsung S24 Ultra"];
      const allowedHonorModels = ["Honor Magic 6", "Honor X10"];
      ...
      // Permitir Pixel y emuladores Pixel
      else if (manufacturer === "Google" && (model.includes("Pixel") || model.includes("Android SDK built"))) {
        compatible = true;
      } 
      // Permitir todos los dispositivos en modo desarrollo
      else if (__DEV__) {
        compatible = true;
      } else {
        compatible = false;
